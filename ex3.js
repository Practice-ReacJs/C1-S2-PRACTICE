...
function getCartTotalAmount() {
  ...
  for (const item of SHOPPING_CART) {
    const product = PRODUCTS.find((subProduct) => subProduct.id === item.id);
    if (product) {
...
function addProductToCart(productId) {
  const checkIndex = SHOPPING_CART.findIndex(
    (item) => item.id === productId
  );
  if (checkIndex !== -1) {
    SHOPPING_CART[checkIndex].quantity++;
  } else {
...
function removeProductFromCart(productId) {
  ...
  if (checkIndex !== -1) {
    if (SHOPPING_CART[checkIndex].quantity > 1) {
      SHOPPING_CART[checkIndex].quantity--;
    } else {
      SHOPPING_CART.splice(checkIndex, 1);
    }
  }
}
